...
async function testPantheonService() {
  // Create a test directory
  const testDir = path.join(process.cwd(), 'test-pantheon');
  await fs.mkdir(testDir, { recursive: true });

  // Initialize service with test directory
  const pantheon = new PantheonService(testDir);

  console.log('Testing Pantheon Service...\n');

  // Test 1: Check if .claude directory exists (should be false initially)
  console.log('1. Checking if Pantheon is initialized:');
  const isInitialized = await pantheon.checkClaudeDirectory();
  console.log(`   Initialized: ${isInitialized}\n`);

  // Test 2: Initialize Pantheon
  console.log('2. Initializing Pantheon:');
  const initResult = await pantheon.initPantheon();
  ...
  console.log('3. Getting Pantheon status:');
  const status = await pantheon.getStatus();
  console.log(`   Status:`, JSON.stringify(status, null, 2), '\n');

  // Test 4: List available commands
  console.log('4. Listing available commands:');
  const commands = await pantheon.listCommands();
  console.log(`   Commands: ${commands.join(', ')}\n`);

  // Test 5: Execute help command
  console.log('5. Executing /help command:');
  const helpResult = await pantheon.executeCommand('/help');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  testPantheonService().catch(console.error);
}