...
export class PantheonService {
  ...
  async initPantheon(projectName?: string): Promise<CommandResult> {
    // This method is no longer used - /gods init goes directly to Claude Code
    return {
      success: true,
      output: 'Please use /gods init command to start an interactive session with Claude Code.',
      artifacts: [{
        type: 'command',
        name: 'gods',
...
- Create apps: "/gods create-app web my-app"
...
`;
  ...
  async executeCommand(commandLine: string): Promise<CommandResult> {
    try {
      ...
      // The baco.md file will tell Claude Code how to handle /gods init
      ...
      // For /gods init, we don't need .claude to exist yet
      if (commandName !== 'baco' && !await this.checkClaudeDirectory()) {
        return {
          success: false,
          error: 'Pantheon is not initialized. Run "/gods init" first.'
        };
      }
