...
class UIDesignAnalyzerServer {
    ...
    async handleFirecrawlScrapeDesign(args) {
    ...
    async handleScreenshotAnalyze(args) {
        ...
        return await limiter.schedule(async () => {
            ...
            try {
                ...
                await page.setViewport(viewports[viewport_size]);
    ...
    async handleUIPatternExtract(args) {
    ...
    async handleDesignSystemGenerate(args) {
    ...
    async handleColorPaletteExtract(args) {
    ...
    async handleLayoutAnalyze(args) {
    ...
    async analyzeWithVision(screenshot, provider) {
        try {
    ...
    extractButtonPatterns(data) {
        ...
        if (data.design && data.design.css) {
            ...
            data.design.css.inline?.forEach((css) => {
    ...
    extractFormPatterns(data) {
        ...
        if (data.markdown) {
            ...
            while ((match = formRegex.exec(data.markdown)) !== null) {
                ...
                if (actionMatch)
                    formPattern.action = actionMatch[1];
                if (methodMatch)
                    formPattern.method = methodMatch[1];
                // Extract inputs
                let inputMatch;
    ...
    generateComponentSpecs(patterns) {
        const specs = {};
        Object.keys(patterns).forEach(key => {
            if (patterns[key] && patterns[key].length > 0) {
                ...
                items.forEach((item) => {
                    ...
                    let variation = spec.variations.find((v) => v.type === variationType);
                    if (!variation) {
                        variation = {
                            type: variationType,
                            instances: [],
                ...
                if (key === 'forms') {
                    ...
                    if (!items.some((i) => i.type === 'validation')) {
        ...
        specs._metadata = {
            ...
            has_consistency_issues: Object.values(specs).some((spec) => spec.variations && spec.variations.length > 5)
        };
        return specs;
    }
    modernizeDesignTokens(tokens) {
        const modernTokens = {
            colors: {},
            spacing: {},
            typography: {},
    ...
    checkColorAccessibility(colors) {
        ...
        for (let i = 0; i < colors.length; i++) {
            for (let j = i + 1; j < colors.length; j++) {
                ...
                if (pair.wcag.AA_normal)
                    results.wcagCompliance.AA.passed++;
                else
                    results.wcagCompliance.AA.failed++;
                if (pair.wcag.AAA_normal)
                    results.wcagCompliance.AAA.passed++;
                else
                    results.wcagCompliance.AAA.failed++;
            }
        }
        ...
        if (results.pairs.length > 0) {
            const avgContrast = results.pairs.reduce((sum, p) => sum + parseFloat(p.contrast), 0) / results.pairs.length;
            if (avgContrast < 4.5) {
        ...
        if (results.pairs.length > 0) {
            results.pairs.sort((a, b) => parseFloat(b.contrast) - parseFloat(a.contrast));
    ...
    suggestUsage(contrast) {
        if (contrast >= 7)
            return 'Excellent for all text sizes';
        if (contrast >= 4.5)
            return 'Good for normal text, excellent for large text';
        if (contrast >= 3)
            return 'Acceptable only for large text (18pt+)';
        return 'Poor contrast - avoid for text';
    }
    analyzeGridSystem(css) {
        const gridInfo = {
            type: 'none',
            columns: 0,
            gutters: [],
            ...
            css.inline?.forEach((style) => {
                ...
                if (flexRegex.test(style)) {
                    ...
                    if (flexDirectionMatch)
                        flexContainer.properties.direction = flexDirectionMatch[1].trim();
                    const justifyMatch = /justify-content:\s*([^;]+)/.exec(style);
                    if (justifyMatch)
                        flexContainer.properties.justify = justifyMatch[1].trim();
                    const alignMatch = /align-items:\s*([^;]+)/.exec(style);
                    if (alignMatch)
                        flexContainer.properties.align = alignMatch[1].trim();
                    const wrapMatch = /flex-wrap:\s*([^;]+)/.exec(style);
                    if (wrapMatch)
                        flexContainer.properties.wrap = wrapMatch[1].trim();
                    gridInfo.flexContainers.push(flexContainer);
                    if (gridInfo.type === 'none')
            ...
            css.inline?.forEach((style) => {
                ...
                while ((match = mediaRegex.exec(style)) !== null) {
                    ...
                    if (minWidthMatch) {
                        ...
                        if (pxValue >= 320 && pxValue <= 480)
                            breakpoint.type = 'mobile';
                        else if (pxValue >= 481 && pxValue <= 768)
                            breakpoint.type = 'tablet';
                        else if (pxValue >= 769 && pxValue <= 1024)
                            breakpoint.type = 'desktop';
                        else if (pxValue >= 1025 && pxValue <= 1440)
                            breakpoint.type = 'wide';
                        else if (pxValue > 1440)
                            breakpoint.type = 'ultra-wide';
    ...
    identifyLayoutPatterns(data) {
        ...
        if (data.structure) {
            ...
            const hasHeader = data.structure.sections?.some((s) => s.tag === 'header' || s.class?.includes('header'));
            ...
            const hasFooter = data.structure.sections?.some((s) => s.tag === 'footer' || s.class?.includes('footer'));
            if (hasFooter) {
            ...
            const hasMain = data.structure.sections?.some((s) => s.tag === 'main' || s.class?.includes('main') || s.class?.includes('content'));
            ...
            const hasSidebar = data.structure.sections?.some((s) => s.tag === 'aside' ||
            ...
            const hasHero = data.structure.sections?.some((s) => s.class?.includes('hero') ||
